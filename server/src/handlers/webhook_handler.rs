...
pub async fn github_webhook(
    ...
    let secret = match crate::repositories::webhook_repository::get_secret(&state.pool, team_id, "github").await {
        Ok(Some(s)) => s,
        Ok(None) => {
            return (
    ...
    let signature = match headers.get("X-Hub-Signature-256").and_then(|v| v.to_str().ok()) {
        Some(sig) => sig,
        None => {
            tracing::warn!("Webhook GitHub reçu sans signature pour team_id={}", team_id);
            return (
                StatusCode::UNAUTHORIZED,
                Json(serde_json::json!(ApiError::new("Signature manquante", "MISSING_SIGNATURE"))),
            );
        }
    };

    if verify_github_signature(&body, signature, &secret).is_err() {
        tracing::warn!("Signature invalide sur webhook GitHub pour team_id={}", team_id);
        return (
            StatusCode::UNAUTHORIZED,
            Json(serde_json::json!(ApiError::new("Signature invalide", "INVALID_SIGNATURE"))),
        );
    }
    ...
    let payload: serde_json::Value = match serde_json::from_slice(&body) {
        ...
        Err(_) => {
            return (
                ...
                Json(serde_json::json!(ApiError::new("Payload JSON invalide", "INVALID_PAYLOAD"))),
            );
        }
    };

    // Transmet au moteur de règles pour évaluation et déclenchement
    rule_engine_service::process_incoming_event(&state, team_id, "github", &event_type, payload).await;

    (
        StatusCode::OK,
        Json(serde_json::json!(ApiResponse::<()>::success_no_data("Webhook traité avec succès"))),
    )
}