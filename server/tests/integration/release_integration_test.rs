...
async fn test_create_release_as_manager_succeeds() {
    ...
    let result =
        release_service::create_release(&pool, team_id, manager_id, make_release_req("v1.0")).await;
...
async fn test_create_release_as_observer_fails() {
    ...
    let result =
        release_service::create_release(&pool, team_id, observer_id, make_release_req("v1.0"))
            .await;
...
async fn test_start_release_succeeds() {
    ...
    let release =
        release_service::create_release(&pool, team_id, manager_id, make_release_req("v1.0"))
            .await
            .unwrap();
...
async fn test_validate_steps_in_order() {
    ...
    let release =
        release_service::create_release(&pool, team_id, manager_id, make_release_req("v1.0"))
            .await
            .unwrap();
    release_service::start_release(&pool, release.id, manager_id)
        .await
        .unwrap();
...
async fn test_incident_blocks_release() {
    ...
    let blocked = release_service::block_release_if_needed(&pool, release.id, incident.id)
        .await
        .unwrap();
    assert!(blocked);

    let updated = release_service::get_release(&pool, release.id, manager_id)
        .await
        .unwrap();
...
async fn test_resolving_incident_unblocks_release() {
    ...
    release_service::block_release_if_needed(&pool, release.id, incident.id)
        .await
        .unwrap();
    ...
    let unblocked = release_service::unblock_release_if_resolved(&pool, release.id)
        .await
        .unwrap();
    assert!(unblocked);
