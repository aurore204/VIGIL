...
async fn test_create_user_then_find_by_email_returns_it() {
    ...
    let created = user_repository::create_user(
        &pool,
        &email,
...
async fn test_invalidate_tokens_sets_invalidated_at() {
    ...
    let created =
        user_repository::create_user(&pool, &email, &format!("testuser4_{}", suffix), "hash")
            .await
            .unwrap();
...
async fn test_is_token_valid_returns_true_before_invalidation() {
    ...
    let created =
        user_repository::create_user(&pool, &email, &format!("testuser5_{}", suffix), "hash")
            .await
            .unwrap();
...
async fn test_is_token_valid_returns_false_after_invalidation() {
    ...
    let created =
        user_repository::create_user(&pool, &email, &format!("testuser6_{}", suffix), "hash")
            .await
            .unwrap();
...
async fn test_update_user_updates_only_provided_fields() {
    ...
    let created =
        user_repository::create_user(&pool, &email, &format!("nom_original_{}", suffix), "hash")
            .await
            .unwrap();
...
async fn test_update_user_updates_language() {
    ...
    let created =
        user_repository::create_user(&pool, &email, &format!("user_lang_{}", suffix), "hash")
            .await
            .unwrap();
