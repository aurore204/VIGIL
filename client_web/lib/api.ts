...
export const api = {
  ...
  generateInvitation: (teamId: string) =>
    request<{ code: string }>(`/teams/${teamId}/invitations`, {
      method: "POST",
    }),
  ...
  acknowledgeIncident: (incidentId: string) =>
    request<Incident>(`/incidents/${incidentId}/acknowledge`, {
      method: "PATCH",
    }),
  ...
  removeReaction: (incidentId: string, entryId: string, emoji: string) =>
    request<void>(
      `/incidents/${incidentId}/timeline/${entryId}/reactions/${emoji}`,
      {
        method: "DELETE",
      },
    ),
  ...
  // Rules (Phase 2)
  getTeamRules: (teamId: string) => request<Rule[]>(`/teams/${teamId}/rules`),
