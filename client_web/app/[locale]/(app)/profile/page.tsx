...
export default function ProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (message.includes('EMAIL_ALREADY_EXISTS') || message.includes('existe déjà') || message.includes('already exists')) {
      ...
      } else if (message.includes('INVALID_CURRENT_PASSWORD') || message.includes('incorrect')) {
  ...
  const langButtonStyle = (isActive: boolean): React.CSSProperties => ({
    ...
    fontSize: '13px', fontWeight: 600, cursor: changingLanguage ? 'not-allowed' : 'pointer',
    opacity: changingLanguage ? 0.6 : 1,
    fontFamily: 'Inter, system-ui, sans-serif',
  });

  return (
    <div style={{ padding: '28px 32px', maxWidth: '560px', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div style={{ marginBottom: '24px' }}>
        <div style={{ fontSize: '22px', fontWeight: 700, color: 'oklch(0.95 0.005 260)' }}>{t('title')}</div>
        <div style={{ fontSize: '13px', color: 'oklch(0.60 0.01 260)', marginTop: '4px' }}>{t('subtitle')}</div>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: '13px', fontWeight: 700, color: 'oklch(0.90 0.005 260)' }}>
              {t('passwordSectionTitle')}
            </div>
            <div style={{ fontSize: '11.5px', color: 'oklch(0.55 0.01 260)', marginTop: '3px' }}>
        ...
        <div style={{ fontSize: '11.5px', color: 'oklch(0.52 0.012 260)' }}>
          {t('memberSince')} {new Date(user.created_at).toLocaleDateString(dateLocale)}
        </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '11.5px', color: 'oklch(0.55 0.01 260)', marginTop: '3px', marginBottom: '14px' }}>
