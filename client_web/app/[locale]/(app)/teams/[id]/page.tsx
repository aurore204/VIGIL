...
export default function TeamDetailPage() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- chargement initial des données de la team au montage
    load();
    api
      .getOnlineUsers()
  ...
  const confirmAction = async () => {
    ...
    try {
      if (confirm.action === "kick") {
        ...
        const expiresAt =
          banType === "temporary" && banDate
            ? new Date(banDate).toISOString()
            : undefined;
        ...
        showToast(
          t("toastTransferred", { username: confirm.target }),
          "success",
        );
  ...
  const confirmDescription = () => {
    ...
    switch (confirm.action) {
      case "kick":
        return t("confirmDialog.kickDescription", { target: confirm.target });
      case "ban":
        return t("confirmDialog.banDescription", { target: confirm.target });
      case "transfer":
        return t("confirmDialog.transferDescription", {
          target: confirm.target,
        });
      case "leave":
        return t("confirmDialog.leaveDescription", { target: confirm.target });
      default:
        return t("confirmDialog.deleteDescription", { target: confirm.target });
    }
  };
        ...
        {isManager && (
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            ...
            <Button
              variant="danger"
              onClick={() =>
                setConfirm({ action: "delete", target: team.name, userId: "" })
              }
            >
              {t("delete")}
            </Button>
          ...
          >
            <MemberRow
              ...
              onKick={(userId, username) =>
                setConfirm({ action: "kick", target: username, userId })
              }
              onBan={(userId, username) =>
                setConfirm({ action: "ban", target: username, userId })
              }
              onTransfer={(userId, username) =>
                setConfirm({ action: "transfer", target: username, userId })
              }
              onLeave={() =>
                setConfirm({ action: "leave", target: team.name, userId: "" })
              }
            />
          </div>
        ))}
