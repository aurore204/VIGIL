      ...
      <div style={{ padding: "17px 18px" }}>
        ...
        <div
          style={{
            ...
            color: iconColor === COLORS.emberText ? iconColor : COLORS.text,
          }}
        >
          {value}
        </div>
...
export default function DashboardPage() {
  ...
  const activeReleases = releases.filter(
    (r) => r.state === "in_progress" || r.state === "blocked",
  );
  const roleOf = (team: Team) =>
  ...
  const activityFeed: ActivityItem[] = [
    ...incidents.slice(0, 5).map((i): ActivityItem => ({
      ...
      kind: "incident",
      label: i.title,
      detail: t("activityCard.incidentDetail", {
        state: tIncidentState(i.state).toLowerCase(),
      }),
      at: i.updated_at,
    })),
    ...releases.slice(0, 5).map((r): ActivityItem => ({
      ...
      kind: "release",
      label: r.title,
      detail: t("activityCard.releaseDetail", {
        state: tReleaseState(r.state).toLowerCase(),
      }),
      at: r.updated_at,
    })),
  ]
    ...
    >
      ...
      >
        ...
        <StatCard
          ...
          stripColor={COLORS.emberText}
          iconBg={COLORS.emberBg}
          iconColor={COLORS.emberText}
        />
        <StatCard
          Icon={Rocket}
          value={activeReleases.length}
          label={t("stats.activeReleases")}
          stripColor={COLORS.greenStrip}
        ...
        <div style={cardStyle}>
          ...
          <div style={{ padding: "17px 20px" }}>
            ...
            {activeIncidents.length === 0 ? (
              ...
              >
                {t("incidentsCard.empty")}
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column" }}>
                {activeIncidents.slice(0, 5).map((incident, i, arr) => {
                  const cfg =
                    stateIconConfig[incident.state] ?? stateIconConfig.open;
        ...
        <div style={cardStyle}>
          ...
          <div style={{ padding: "17px 20px" }}>
            ...
            >
              ...
              <span style={{ fontSize: "11px", color: COLORS.greenText }}>
                {t("onlineCard.activeCount", { count: onlineUsernames.length })}
              </span>
            </div>
            ...
            {activeReleases.length === 0 ? (
              ...
              >
                {activeReleases.slice(0, 3).map((release) => {
                  const completed = release.steps.filter(
                    (s) => s.state === "completed",
                  ).length;
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          {blocked
                            ? tReleaseState("blocked")
                            : tReleaseState("in_progress")}
                        </span>
                      </div>
                      <div
                        style={{
                      ...
                      >
                        {t("releasesCard.stepsProgress", {
                          completed,
                          total: release.steps.length,
                        })}
                      </div>
                    </Link>
                  );
      ...
      <div>
        ...
        >
          <div style={{ fontSize: "14px", fontWeight: 600 }}>
            {t("teamsSection.title", { count: teams.length })}
          </div>
          <Link
            href="/teams"
            style={{
