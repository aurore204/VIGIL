...
jest.mock('@/components/incidents/IncidentTable', () => ({
  IncidentTable: ({ incidents }: { incidents: Incident[] }) => (
    <div data-testid="incident-table">{incidents.map(i => <div key={i.id}>{i.title}</div>)}</div>
  ),
}));
...
jest.mock('@/components/incidents/CreateIncidentModal', () => ({
  CreateIncidentModal: ({ onClose, onSubmit }: any) => (
    <div data-testid="create-modal">
      ...
      <button onClick={() => onSubmit('team-1', 'Nouveau titre', 'high', undefined)}>submit-modal</button>
    </div>
  ),
}));
...
const makeTeam = (overrides: Partial<Team> = {}): Team => ({
  ...
  members: [], created_at: '2026-01-01T00:00:00Z',
  ...overrides,
});
...
describe('IncidentsPage', () => {
  ...
  it('affiche les incidents chargés', async () => {
    ...
    (api.getIncidents as jest.Mock).mockResolvedValue([makeIncident({ title: 'Incident A' })]);
  ...
  it('filtre les incidents par recherche texte', async () => {
    ...
    render(<IncidentsPage />);
    await waitFor(() => expect(screen.getByText('Panne réseau')).toBeInTheDocument());

    await userEvent.type(screen.getByPlaceholderText('searchPlaceholder'), 'réseau');
  ...
  it('filtre les incidents par sévérité', async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam()]);
    (api.getIncidents as jest.Mock).mockResolvedValue([
      makeIncident({ id: 'inc-1', title: 'Incident critique', severity: 'critical' }),
    ...
    await userEvent.selectOptions(screen.getByLabelText('severityLabel'), 'critical');
  ...
  it('filtre les incidents par état', async () => {
    ...
    (api.getIncidents as jest.Mock).mockResolvedValue([
      ...
      makeIncident({ id: 'inc-2', title: 'Incident résolu', state: 'resolved' }),
    ]);

    render(<IncidentsPage />);
    await waitFor(() => expect(screen.getByText('Incident ouvert')).toBeInTheDocument());

    await userEvent.selectOptions(screen.getByLabelText('stateLabel'), 'resolved');
  ...
  it("n'affiche pas le bouton de création si l'utilisateur n'est manager d'aucune team", async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam({ manager_id: 'quelquun-dautre' })]);
  ...
  it('affiche le modal de création et le ferme au clic sur close', async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam({ manager_id: 'user-1' })]);
  ...
  it('crée un incident et affiche un toast de succès', async () => {
    ...
    await waitFor(() => {
      expect(api.createIncident).toHaveBeenCalledWith('team-1', { title: 'Nouveau titre', severity: 'high', description: undefined });
  ...
  it('enregistre les handlers WebSocket au montage et les retire au démontage', async () => {
    ...
    expect(vigilWs.on).toHaveBeenCalledWith('incident_state_changed', expect.any(Function));

    unmount();

    expect(vigilWs.off).toHaveBeenCalledWith('incident_state_changed', expect.any(Function));
  });
});