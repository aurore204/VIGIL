...
jest.mock('@/components/rules/CreateRuleModal', () => ({
  CreateRuleModal: ({ onClose }: any) => (
    <div data-testid="create-rule-modal"><button onClick={onClose}>close-rule-modal</button></div>
  ),
}));

jest.mock('@/components/rules/WebhookSecretModal', () => ({
  WebhookSecretModal: ({ onClose }: any) => (
    <div data-testid="webhook-modal"><button onClick={onClose}>close-webhook-modal</button></div>
  ),
}));
...
const makeRule = (overrides: Partial<Rule> = {}): Rule => ({
  id: 'rule-1', team_id: 'team-1', created_by: 'user-1', name: 'Ma règle', enabled: true,
...
describe('RulesPage', () => {
  ...
  it('charge automatiquement les règles de la première team manager', async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam({ id: 'team-1', manager_id: 'user-1' })]);
    (api.getTeamRules as jest.Mock).mockResolvedValue([makeRule({ name: 'Règle CI' })]);
