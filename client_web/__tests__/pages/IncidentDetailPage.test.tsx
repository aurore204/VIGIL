...
jest.mock('@/components/incidents/IncidentActions', () => ({
  IncidentActions: ({ onAcknowledge, onEscalate, onResolve, onAssign, onEdit, onDelete }: any) => (
...
jest.mock('@/components/incidents/EditIncidentModal', () => ({
  EditIncidentModal: ({ onSubmit, onClose }: any) => (
    <div data-testid="edit-modal">
      <button onClick={() => onSubmit({ title: 'Titre édité' })}>confirm-edit</button>
...
const makeIncident = (overrides: Partial<Incident> = {}): Incident => ({
  ...
  title: 'Panne serveur', description: 'Une description', state: 'open', severity: 'high',
...
describe('IncidentDetailPage', () => {
  ...
  it('ouvre le modal edit et appelle updateIncident à la confirmation', async () => {
    ...
    expect(api.updateIncident).toHaveBeenCalledWith('inc-1', { title: 'Titre édité' });
  });
  ...
  it('affiche un toast d\'erreur si acknowledgeIncident échoue', async () => {
    (api.acknowledgeIncident as jest.Mock).mockRejectedValue(new Error('Erreur serveur'));
    render(<IncidentDetailPage />);
    await waitFor(() => expect(screen.queryByText('loading')).not.toBeInTheDocument());
