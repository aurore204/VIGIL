...
jest.mock('@/components/releases/ReleaseCard', () => ({
  ReleaseCard: ({ release, teamName }: any) => <div>{release.title} — {teamName}</div>,
}));
...
jest.mock('@/components/releases/CreateReleaseModal', () => ({
  CreateReleaseModal: ({ onClose, onSubmit }: any) => (
    <div data-testid="create-modal">
      ...
      <button onClick={() => onSubmit('team-1', 'Nouvelle release', ['build', 'deploy'], undefined)}>submit-modal</button>
    </div>
  ),
}));
...
describe('ReleasesPage', () => {
  ...
  it('affiche les releases chargées avec le nom de leur team', async () => {
    ...
    (api.getReleases as jest.Mock).mockResolvedValue([makeRelease({ title: 'Release X' })]);
  ...
  it('enregistre les handlers WebSocket au montage et les retire au démontage', async () => {
    ...
    expect(vigilWs.on).toHaveBeenCalledWith('release_state_changed', expect.any(Function));

    unmount();

    expect(vigilWs.off).toHaveBeenCalledWith('release_state_changed', expect.any(Function));
  });
});