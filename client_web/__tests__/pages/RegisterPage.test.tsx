...
describe('RegisterPage', () => {
  ...
  it('affiche tous les champs du formulaire', () => {
    ...
    expect(screen.getAllByPlaceholderText('register.passwordPlaceholder').length).toBe(2);
  });

  it('affiche une erreur si les mots de passe ne correspondent pas', async () => {
    render(<RegisterPage />);

    await userEvent.type(screen.getByPlaceholderText('register.usernamePlaceholder'), 'alice');
    await userEvent.type(screen.getByPlaceholderText('register.emailPlaceholder'), 'alice@test.com');
    const passwordFields = screen.getAllByPlaceholderText('register.passwordPlaceholder');
  ...
  it('appelle api.register puis redirige vers /auth/login en cas de succès', async () => {
    ...
    expect(api.register).toHaveBeenCalledWith('alice@test.com', 'password123', 'alice');
    expect(mockPush).toHaveBeenCalledWith('/auth/login');
  });

  it('affiche un message d\'erreur si l\'inscription échoue (ex: email déjà utilisé)', async () => {
    (api.register as jest.Mock).mockRejectedValue(new Error('Un compte avec cet email existe déjà'));
    ...
    expect(await screen.findByText('Un compte avec cet email existe déjà')).toBeInTheDocument();
