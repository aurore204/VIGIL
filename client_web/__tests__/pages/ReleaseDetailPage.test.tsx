...
jest.mock('@/components/shared/PresenceIndicator', () => ({
  PresenceIndicator: ({ watchers }: { watchers: string[] }) => <div data-testid="presence">{watchers.join(',')}</div>,
}));
...
const makeStep = (overrides: any = {}) => ({
  id: 's1', release_id: 'rel-1', name: 'build', description: null, position: 0,
  state: 'pending', validated_by: null, validated_at: null,
...
describe('ReleaseDetailPage', () => {
  ...
  it("n'affiche pas les boutons de gestion pour un non-manager", async () => {
    (api.getTeam as jest.Mock).mockResolvedValue(makeTeam({
      members: [{ user_id: 'user-1', username: 'alice', email: 'alice@test.com', role: 'observer', joined_at: '2026-01-01T00:00:00Z' }],
    }));
  ...
  it('affiche le bouton "validate" pour l\'étape courante en cours', async () => {
    ...
    (api.getTeam as jest.Mock).mockResolvedValue(makeTeam({
      members: [{ user_id: 'user-1', username: 'alice', email: 'alice@test.com', role: 'responder', joined_at: '2026-01-01T00:00:00Z' }],
    }));
  ...
  it('affiche l\'avertissement quand la release est bloquée', async () => {
    (api.getRelease as jest.Mock).mockResolvedValue(makeRelease({ state: 'blocked' }));
