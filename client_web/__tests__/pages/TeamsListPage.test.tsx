...
describe('TeamsPage', () => {
  ...
  it('ouvre le modal de création et crée une team', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText('createModal.namePlaceholder'), 'Nouvelle Team');
  ...
  it('ouvre le modal de jointure et rejoint une team avec un code', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText('joinModal.codePlaceholder'), 'ABC12345');
  ...
  it('affiche un toast d\'erreur générique si le code de jointure est invalide', async () => {
    ...
    await userEvent.click(screen.getAllByText('join')[0]);
    await userEvent.type(screen.getByPlaceholderText('joinModal.codePlaceholder'), 'BADCODE');
    await userEvent.click(screen.getByText('joinModal.submit'));
  ...
  it('calcule le total d\'incidents actifs toutes teams confondues', async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam({ id: 'team-1' }), makeTeam({ id: 'team-2' })]);
    (api.getIncidents as jest.Mock).mockImplementation((teamId: string) =>
      Promise.resolve(teamId === 'team-1' ? [{ state: 'open' }] : [{ state: 'resolved' }])
    );
  ...
  it('enregistre les handlers WebSocket au montage', async () => {
    render(<TeamsPage />);
    await waitFor(() => {
      expect(vigilWs.on).toHaveBeenCalledWith('member_kicked', expect.any(Function));
    });
  });
});