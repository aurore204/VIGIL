...
const makeTeam = (overrides: Partial<Team> = {}): Team => ({
  ...
  members: [{ user_id: 'user-1', username: 'alice', email: 'alice@test.com', role: 'manager', joined_at: '2026-01-01T00:00:00Z' }],
...
const makeRelease = (overrides: Partial<Release> = {}): Release => ({
  id: 'rel-1', team_id: 'team-1', created_by: 'user-1', title: 'Release v1', description: null,
  state: 'in_progress',
  steps: [{ id: 's1', release_id: 'rel-1', name: 'build', description: null, position: 0, state: 'completed', validated_by: null, validated_at: null, created_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z' }],
...
describe('DashboardPage', () => {
  ...
  it('affiche le nombre d\'incidents actifs dans la stat card', async () => {
    ...
    (api.getIncidents as jest.Mock).mockResolvedValue([makeIncident({ state: 'open' }), makeIncident({ id: 'inc-2', state: 'resolved' })]);
  ...
  it('affiche le titre de chaque incident actif', async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam()]);
    (api.getIncidents as jest.Mock).mockResolvedValue([makeIncident({ title: 'Panne critique' })]);
  ...
  it('affiche les releases actives (in_progress ou blocked)', async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam()]);
    (api.getReleases as jest.Mock).mockResolvedValue([
      makeRelease({ id: 'rel-1', title: 'Release active', state: 'in_progress' }),
      makeRelease({ id: 'rel-2', title: 'Release terminée', state: 'completed' }),
    ]);
  ...
  it('affiche les teams de l\'utilisateur', async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam({ name: 'Team Alpha' })]);
  ...
  it('rend un lien vers la page de détail de chaque team', async () => {
    (api.getTeams as jest.Mock).mockResolvedValue([makeTeam({ id: 'team-99' })]);

    render(<DashboardPage />);

    await waitFor(() => {
      const links = screen.getAllByRole('link').filter(l => l.getAttribute('href') === '/teams/team-99');
  ...
  it('désinscrit les handlers WebSocket au démontage', async () => {
    ...
    expect(vigilWs.off).toHaveBeenCalledWith('presence_online', expect.any(Function));
  });
});
