...
jest.mock("@/components/teams/MemberRow", () => ({
  MemberRow: ({ member, onKick, onBan, onTransfer, onLeave }: any) => (
    <div>
      ...
      <button onClick={() => onKick(member.user_id, member.username)}>
        kick-{member.username}
      </button>
      <button onClick={() => onBan(member.user_id, member.username)}>
        ban-{member.username}
      </button>
      <button onClick={() => onTransfer(member.user_id, member.username)}>
        transfer-{member.username}
      </button>
...
jest.mock("@/components/teams/BannedMemberRow", () => ({
  BannedMemberRow: ({ banned, onUnban }: any) => (
    <div>
      ...
      <button onClick={() => onUnban(banned.user_id, banned.username)}>
        unban-{banned.username}
      </button>
    </div>
  ),
}));
...
describe("TeamDetailPage", () => {
  ...
  it("génère un code d'invitation et l'affiche", async () => {
    (api.generateInvitation as jest.Mock).mockResolvedValue({
      code: "INVITE123",
    });

    render(<TeamDetailPage />);
    await waitFor(() =>
      expect(screen.getByText("generateCode")).toBeInTheDocument(),
    );
  ...
  it("ouvre la confirmation puis kick un membre", async () => {
    ...
    render(<TeamDetailPage />);
    await waitFor(() =>
      expect(screen.getByText("kick-bob")).toBeInTheDocument(),
    );
  ...
  it("ouvre la confirmation puis transfère le rôle manager", async () => {
    ...
    render(<TeamDetailPage />);
    await waitFor(() =>
      expect(screen.getByText("transfer-bob")).toBeInTheDocument(),
    );
  ...
  it("unban un membre", async () => {
    ...
    render(<TeamDetailPage />);
    await waitFor(() =>
      expect(screen.getByText("unban-charlie")).toBeInTheDocument(),
    );
