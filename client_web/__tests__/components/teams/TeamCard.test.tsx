...
describe('TeamCard', () => {
  it('affiche le nom de la team', () => {
    render(<TeamCard team={makeTeam()} activeIncidents={0} currentUserId="user-1" />);
  ...
  it("affiche un badge d'incidents actifs quand activeIncidents > 0", () => {
    render(<TeamCard team={makeTeam()} activeIncidents={3} currentUserId="user-1" />);
  ...
  it('affiche les initiales de chaque membre (max 5)', () => {
    const manyMembers = Array.from({ length: 7 }, (_, i) => ({
      user_id: `user-${i}`, username: `member${i}`, email: `m${i}@test.com`,
      role: 'observer' as const, joined_at: '2026-01-01T00:00:00Z',
    }));
    render(<TeamCard team={makeTeam({ members: manyMembers })} activeIncidents={0} currentUserId="user-0" />);
  ...
  it('rend un lien vers la page de détail de la team', () => {
    render(<TeamCard team={makeTeam({ id: 'team-42' })} activeIncidents={0} currentUserId="user-1" />);
