...
describe('BannedMemberRow', () => {
  it("affiche le nom d'utilisateur banni", () => {
    render(<BannedMemberRow banned={makeBanned()} canUnban={false} onUnban={jest.fn()} />);
  ...
  it('affiche "permanently" quand expires_at est null', () => {
    render(<BannedMemberRow banned={makeBanned({ expires_at: null })} canUnban={false} onUnban={jest.fn()} />);
    expect(screen.getByText(/permanently/)).toBeInTheDocument();
  });

  it('affiche "until" avec une date quand expires_at est présent', () => {
    render(<BannedMemberRow banned={makeBanned({ expires_at: '2026-06-01T12:00:00Z' })} canUnban={false} onUnban={jest.fn()} />);
    expect(screen.getByText(/until/)).toBeInTheDocument();
  });

  it("n'affiche pas de raison si elle est absente", () => {
    render(<BannedMemberRow banned={makeBanned({ reason: null })} canUnban={false} onUnban={jest.fn()} />);
    expect(screen.queryByText(/reason/)).not.toBeInTheDocument();
  });

  it('affiche la raison quand elle est présente', () => {
    render(<BannedMemberRow banned={makeBanned({ reason: 'comportement inapproprié' })} canUnban={false} onUnban={jest.fn()} />);
  ...
  it('affiche le bouton unban quand canUnban est true', () => {
    render(<BannedMemberRow banned={makeBanned()} canUnban onUnban={jest.fn()} />);
  ...
  it('appelle onUnban avec le bon user_id et username', async () => {
    ...
    render(<BannedMemberRow banned={makeBanned({ user_id: 'user-42', username: 'charlie' })} canUnban onUnban={onUnban} />);
