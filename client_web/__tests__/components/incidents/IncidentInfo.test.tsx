...
describe('IncidentInfo', () => {
  ...
  it("affiche le nom d'utilisateur de la personne assignée", () => {
    const team = makeTeam({
      members: [{ user_id: 'user-1', username: 'bob', email: 'bob@test.com', role: 'responder', joined_at: '2026-01-01T00:00:00Z' }],
    });
  ...
  it('affiche "unassigned" quand aucun assigné', () => {
    render(<IncidentInfo incident={makeIncident({ assigned_to: null })} team={makeTeam()} />);

    expect(screen.getByText('unassigned')).toBeInTheDocument();
  });

  it("n'affiche pas la date de résolution si l'incident n'est pas résolu", () => {
    render(<IncidentInfo incident={makeIncident({ resolved_at: null })} team={makeTeam()} />);
