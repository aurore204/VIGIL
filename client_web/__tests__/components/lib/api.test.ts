...
describe('api — requêtes incidents', () => {
  ...
  it('acknowledgeIncident envoie un PATCH', async () => {
    mockFetch.mockReturnValue(mockResponse({ id: 'inc-1', state: 'acknowledged' }));
  ...
  it('lance une erreur exploitable quand la réponse n\'est pas ok', async () => {
    mockFetch.mockReturnValue(
      Promise.resolve({ ok: false, json: () => Promise.resolve({ error: 'Incident introuvable' }) })
    );
    await expect(api.getIncident('inc-inconnu')).rejects.toThrow('Incident introuvable');
  });
  ...
  it('removeReaction envoie un DELETE avec l\'emoji dans l\'URL', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/incidents/inc-1/timeline/entry-1/reactions/fire'),
  ...
  it('updateIncident envoie un PATCH avec les champs modifiés', async () => {
    mockFetch.mockReturnValue(mockResponse({ id: 'inc-1', title: 'Nouveau titre' }));
