...
describe('CreateRuleModal', () => {
  ...
  it('affiche le champ nom de règle', () => {
    render(<CreateRuleModal teamId="team-1" onClose={jest.fn()} onCreated={jest.fn()} />);
    expect(screen.getByLabelText(/nameLabel/)).toBeInTheDocument();
  });

  it('appelle onClose au clic sur annuler', async () => {
    const onClose = jest.fn();
    render(<CreateRuleModal teamId="team-1" onClose={onClose} onCreated={jest.fn()} />);
  ...
  it('crée une règle vigil_create_incident avec les valeurs par défaut', async () => {
    ...
    render(<CreateRuleModal teamId="team-1" onClose={jest.fn()} onCreated={onCreated} />);

    await userEvent.type(screen.getByLabelText(/nameLabel/), 'Ma règle CI');
    await userEvent.click(screen.getByText('submit'));

    expect(api.createRule).toHaveBeenCalledWith('team-1', expect.objectContaining({
      name: 'Ma règle CI',
      enabled: true,
      trigger: { service: 'github', event: 'workflow_run', filters: { conclusion: 'failure' } },
  ...
  it('crée une règle http_post avec l\'URL fournie', async () => {
    ...
    await userEvent.selectOptions(screen.getByLabelText(/reactionLabel/), 'http_post');
    await userEvent.type(screen.getByLabelText(/httpUrlLabel/), 'https://example.com/hook');
    await userEvent.click(screen.getByText('submit'));

    expect(api.createRule).toHaveBeenCalledWith('team-1', expect.objectContaining({
      reaction: { type: 'http_post', payload: { url: 'https://example.com/hook' } },
    }));
  });
  ...
  it('affiche un toast d\'erreur si la création échoue', async () => {
    (api.createRule as jest.Mock).mockRejectedValue(new Error('Erreur serveur'));
    render(<CreateRuleModal teamId="team-1" onClose={jest.fn()} onCreated={jest.fn()} />);
  ...
  it('décoche includeRepoName retire le suffixe du titre construit', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/nameLabel/), 'Ma règle');
    const checkbox = screen.getByText('includeRepoName').closest('label')!.querySelector('input')!;
    await userEvent.click(checkbox);
    await userEvent.click(screen.getByText('submit'));

    const call = (api.createRule as jest.Mock).mock.calls[0][1];
    expect(call.reaction.payload.title).not.toContain('sur {{repository.name}}');
  });
});