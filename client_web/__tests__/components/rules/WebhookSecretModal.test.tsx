...
describe('WebhookSecretModal', () => {
  ...
  it('copie le chemin du webhook dans le presse-papier au clic', async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith('/webhooks/github/team-42');
  ...
  it('appelle createWebhookSecret avec les bonnes valeurs', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/secretLabel/), 'mon-secret-webhook');
    await userEvent.click(screen.getByText('submit'));

    expect(api.createWebhookSecret).toHaveBeenCalledWith('team-1', 'github', 'mon-secret-webhook');
  });
  ...
  it('affiche un toast d\'erreur si la sauvegarde échoue', async () => {
    (api.createWebhookSecret as jest.Mock).mockRejectedValue(new Error('Secret invalide'));
    render(<WebhookSecretModal teamId="team-1" onClose={jest.fn()} />);
