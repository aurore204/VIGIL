  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        <div style={rowStyle}>
          <span style={labelStyle}><User size={12} aria-hidden="true" />Assigné à</span>
          {assignee ? (
        ...
        <div style={rowStyle}>
          <span style={labelStyle}><Users size={12} aria-hidden="true" />Team</span>
          <span style={valueStyle}>{team?.name ?? '-'}</span>
        </div>
        <div style={rowStyle}>
          <span style={labelStyle}><Calendar size={12} aria-hidden="true" />Créé le</span>
          <span style={valueStyle}>{new Date(incident.created_at).toLocaleString('fr-FR')}</span>
        </div>
        {incident.resolved_at && (
          <div style={rowStyle}>
            <span style={labelStyle}><CheckCircle2 size={12} aria-hidden="true" />Résolu le</span>
