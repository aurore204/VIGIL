...
interface CreateReleaseModalProps {
  teams: Team[];
  onClose: () => void;
  onSubmit: (teamId: string, title: string, steps: string[], description?: string) => Promise<void>;
}

export function CreateReleaseModal({ teams, onClose, onSubmit }: CreateReleaseModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await onSubmit(teamId, title.trim(), cleanedSteps, description.trim() || undefined);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal title={t('title')} onClose={onClose} maxWidth="480px">
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div>
          <label style={labelStyle}>{t('steps')} <span style={{ color: 'oklch(0.78 0.14 25)' }}>*</span></label>
                ...
                {steps.length > 1 && (
                  <button
                    ...
                    style={{
                      ...
                      color: 'oklch(0.55 0.01 260)', flexShrink: 0, padding: '4px',
          ...
          <button
            ...
            style={{
              ...
              background: 'none', border: 'none', cursor: 'pointer',
              color: 'oklch(0.66 0.16 255)', fontSize: '12.5px', fontWeight: 600,
          ...
          {cleanedSteps.length === 0 && (
            <div style={{ fontSize: '11.5px', color: 'oklch(0.78 0.14 25)', marginTop: '6px' }}>
              {t('stepsEmpty')}
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '4px' }}>
          ...
          <Button type="submit" loading={submitting} disabled={!canSubmit}>{t('submit')}</Button>
        </div>
      </form>
    </Modal>
  );
